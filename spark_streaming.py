...
def process_batch(batch_df, batch_id):
    ...
    WHEN NOT MATCHED AND s.op='c' THEN INSERT (id,provider,quantity,gd_barcode,gd_name,p_date,invoice_id,ncode_masked,mobile_masked,year,month,day,city,province,m_date,latitude,longitude,province_code,month_name,gd_cat,gd_brand,last_updated) VALUES (s.id,s.provider,s.quantity,s.gd_barcode,s.gd_name,s.p_date,s.invoice_id,trim(s.ncode_masked),trim(s.mobile_masked),s.year,s.month,s.day,s.city,s.province,s.m_date,s.latitude,s.longitude,s.province_code,s.month_name,s.gd_cat,s.gd_brand,current_timestamp())
    """)
